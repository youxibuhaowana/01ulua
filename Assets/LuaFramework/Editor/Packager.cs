...
public class Packager {
    ...
    public static void BuildAssetResource(BuildTarget target) {
        ...
        //  resPath = Assets/StreamingAssets, 直接生成bundle文件
    ...
    //Q: 为什么资源打包文件路径是:Assets/Lua/Common/define.lua.byte？ lua文件夹是个临时文件夹, 过会儿就删除了?
    //A: assetbundle文件是源文件一种专有压缩，打包后，哪怕将源文件删除掉也不会影响ab的使用
    ...
    static void HandleLuaBundle() {
        ...
        for (int i = 0; i < srcDirs.Length; i++) {
            if (AppConst.LuaByteMode) {
                ...
                // 将sourceDir下所有文件+.bytes  ---复制到--->  destDir下面
                ToLuaMenu.CopyLuaBytesFiles(srcDirs[i], streamDir);
            }
        }
        // 注册资源 assetsbundle 父级目录作为key(有多个子对象)
        ...
        //-------------------------------处理非Lua文件----------------------------------
        // 将非lua文件复制到 /Assets/StreamingAssets/lua/ 下
